...
import { KC, SEPIN_TORPAQ, faza, kcTap, novbetiAy, teqvimQur } from "./teqvim.js";
...
describe("səpin üçün torpaq həddi", () => {
  // Buğda oktyabr-noyabrda səpilir: bu aylarda hədd verilməlidir
  it("səpin ayında həddi qaytarır", () => {
    const teqvim = teqvimQur("bugda", 10);
    expect(teqvim.sepin.aktiv).toBe(true);
    expect(teqvim.sepin.torpaqMin).toBe(SEPIN_TORPAQ.bugda);
  });

  it("səpin ayı olmayanda aktiv deyil", () => {
    expect(teqvimQur("bugda", 5).sepin.aktiv).toBe(false);
  });

  // Hədd bilinmirsə uydurmuruq — üzüm və alma üçün "səpin" anlayışı yoxdur
  it("həddi olmayan bitkidə null qaytarır", () => {
    expect(teqvimQur("uzum", 3).sepin.torpaqMin).toBeNull();
  });
});
