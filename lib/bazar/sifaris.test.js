...
describe("setirleriYoxla — klient girişi", () => {
  ...
  it("klientin göndərdiyi qiymət/cəmi/təchizatçı OXUNMUR", () => {
    ...
    expect(hesab.setirler[0].vahidQiymet).toBe(39.9);
    expect(hesab.setirler[0].tedarukcu).toBe("agrosupply");
    expect(hesab.araCem).toBe(39.9);
  });
});

describe("sebetiHesabla — yekunlar kataloqdan", () => {
  it("brief nümunəsi: 20 × 39,90 = 798", () => {
    const hesab = hesabla([{ kod: "karbamid-46-50kq", say: 20 }]);
    expect(hesab.setirler[0].cemi).toBe(798);
    expect(hesab.araCem).toBe(798);
    // 798 ≥ AgroSupply pulsuz həddi (500) → çatdırılma 0
    expect(hesab.catdirilma).toBe(0);
    expect(hesab.cemi).toBe(798);
    expect(hesab.sayCemi).toBe(20);
  });

  it("qəpik dəqiqliyi: 3 × 39,90 = 119,70 (üzən nöqtə zibili yoxdur)", () => {
    const hesab = hesabla([{ kod: "karbamid-46-50kq", say: 3 }]);
    expect(hesab.araCem).toBe(119.7);
  });
  ...
  it("çatdırılma tədarükçü başına, pulsuz həddin altında haqq tutulur", () => {
    ...
    const hesab = hesabla([
      { kod: "karbamid-46-50kq", say: 2 }, // 79,80 < 500 → 15
    ...
    expect(hesab.cemi).toBe(79.8 + 220 + 15);
  });

  it("səbət nümunəsi: karbamid 20 + pomidor toxumu 2 = 1 018", () => {
    const hesab = hesabla([
      { kod: "karbamid-46-50kq", say: 20 },
      { kod: "pomidor-toxumu-f1", say: 2 },
    ]);
    expect(hesab.araCem).toBe(1018);
    expect(hesab.catdirilma).toBe(0);
    expect(hesab.cemi).toBe(1018);
  });

  it("maliyyələşdirilə bilən məbləğ yalnız uyğun sətirlərdən yığılır", () => {
    const hesab = hesabla([
      { kod: "karbamid-46-50kq", say: 10 }, // maliyye: true → 399
      { kod: "fungisid-mis-1kq", say: 2 }, // maliyye: false → 48
    ]);
    expect(hesab.maliyyeMeblegi).toBe(399);
