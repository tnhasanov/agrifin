...
const METR_DERECE = KM_DERECE * 1000;

/**
 * Metr → dərəcə: Statistical API-nin resx/resy ÖLÇÜSÜ.
 *
 * ═══ NİYƏ BU FUNKSİYA VAR ═════════════════════════════════════════════
 * resx/resy sorğunun KOORDİNAT SİSTEMİNİN vahidindədir. Biz hər yerdə
 * EPSG:4326 göndəririk — orada vahid DƏRƏCƏdir, metr yox. `resx: 60`
 * yazmaq "60 dərəcə piksel" deməkdir: 10 km-lik kvadrat bir piksele
 * yığılır və Copernicus 400 qaytarır:
 *
 *   "Your request of 9991.58 meters per pixel exceeds the limit 1500.00
 *    meters per pixel of the collection S2L2A"
 *
 * Bu, ətraf müqayisəsini TAMAMİLƏ söndürmüşdü (istehsalda tapıldı).
 * Sahənin öz sorğuları isə səssizcə zədələnirdi: kiçik konturda bir
 * piksel ~200-400 m/piksel verir, hədd aşılmır, sorğu "uğurlu" görünür —
 * amma bütün sahə TƏK piksellə ölçülürdü.
 *
 * Uzunluq dərəcəsi enliyə görə qısalır, ona görə resx kosinusla düzəlir.
 *
 * @param {number} metr istənilən yer ölçüsü (10 m, 60 m, …)
 * @param {number} en   sahənin mərkəzi enliyi
 * @returns {{resx: number, resy: number}|null}
 */
export function olcuDereceye(metr, en) {
  if (!Number.isFinite(metr) || metr <= 0) return null;
  if (!Number.isFinite(en) || Math.abs(en) > 90) return null;
  const kosinus = Math.cos((en * Math.PI) / 180);
  if (!Number.isFinite(kosinus) || Math.abs(kosinus) < 0.01) return null;
  return {
    resx: metr / (METR_DERECE * kosinus),
    resy: metr / METR_DERECE,
  };
}

/** Konturun mərkəzi enliyi — ölçü çevirməsi üçün lazımdır */
export function merkeziEn(noqteler) {
  if (!Array.isArray(noqteler) || noqteler.length === 0) return null;
  const enler = noqteler.map((p) => p?.[0]).filter(Number.isFinite);
  if (enler.length === 0) return null;
  return (Math.max(...enler) + Math.min(...enler)) / 2;
}
