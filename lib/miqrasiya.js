...
function xamMezmunHash(metn) {
  return createHash("sha256").update(metn).digest("hex");
}

/**
 * Git faylı eyni SQL olsa da Windows checkout-da CRLF, Vercel/Linux-da LF
 * ola bilər. Checksum platformanı deyil, miqrasiyanın məzmununu qorumalıdır.
 */
function setirSonlariniNormallasdir(metn) {
  return metn.replace(/\r\n?/g, "\n");
}

export function mezmunHash(metn) {
  return xamMezmunHash(setirSonlariniNormallasdir(metn));
}

/**
 * Hash normallaşdırılmamış köhnə versiyada yazılmış ola bilər. Yalnız eyni
 * mətnin LF/CRLF variantlarını qəbul edirik; həqiqi SQL dəyişikliyi yenə xəta
 * verir. Uyğun köhnə hash tapılanda jurnal kanonik LF hash-a yenilənir.
 */
function kohneSetirSonuHashlari(metn) {
  const lf = setirSonlariniNormallasdir(metn);
  return new Set([xamMezmunHash(lf), xamMezmunHash(lf.replace(/\n/g, "\r\n"))]);
}
  ...
  for (const ad of miqrasiyaFayllari()) {
    ...
    if (olanlar.has(ad)) {
      ...
      if (kohneHash == null) {
        ...
        if (kohneSetirSonuHashlari(metn).has(kohneHash)) {
          // SƏSSİZ OLMAMALIDIR: jurnalda saxlanan checksum-u üstündən yazmaq
          // bu sistemdə ən həssas əməliyyatdır — qorumanın özünə toxunur.
          // Səbəb məşrudur (eyni SQL, fərqli sətir sonu), amma operator bunu
          // build logunda GÖRMƏLİDİR, yoxsa "checksum niyə dəyişdi" sualının
          // cavabı heç yerdə qalmır.
          log(
            `↻ ${ad}: checksum CRLF→LF kanonikləşdirildi (SQL məzmunu eynidir, ` +
              `yalnız sətir sonları fərqli idi)`,
          );
          await sorgu(`UPDATE ${MIQRASIYA_CEDVELI} SET mezmun_hash=$2 WHERE ad=$1`, [ad, hash]);
        } else {
          throw new Error(
    ...
    // ON CONFLICT: eyni branch-a iki deployment paralel gedəndə (məsələn
    // ardıcıl iki push) hər ikisi eyni preview bazasına miqrasiya tətbiq edir.
    // Əmrlərin özü idempotentdir, amma jurnal sətri unikal açardır — qoruma
    // olmasa yarışın uduzan tərəfi build-i uydurma xəta ilə sındırırdı.
    // DO NOTHING, DO UPDATE deyil: qalib sətir olduğu kimi qalır. Bura
    // yalnız fayl jurnalda OLMAYANDA gəlinir, ona görə qarşı tərəfin yazdığı
    // hash onsuz da eyni məzmundan çıxır — üstündən yazmaq isə həqiqi
    // fərqliliyi gizlədə bilərdi.
    await sorgu(
      `INSERT INTO ${MIQRASIYA_CEDVELI} (ad, mezmun_hash) VALUES ($1, $2)
