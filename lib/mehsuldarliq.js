...
/**
 * ═══ CARİ MÖVSÜM QATI (OVERLAY) ═══════════════════════════════════════
 * Bal TARİXİdir: 6 amilin 5-i keçmiş mövsümlərdən gəlir. Ona görə güclü
 * tarixçəsi olan sahə bu mövsüm pis getsə də 80-dən yuxarı qala bilər —
 * riyazi olaraq düzgündür, amma ekranda tək başına "Yüksək" yazmaq
 * YANILDIR: fermer bunu "hər şey qaydasındadır" kimi oxuyur.
 *
 * HƏLL: balı AŞAĞI SALMIRIQ (cari mövsümə 20-30 çəki vermək tarixçəni
 * bir aylıq ölçmə ilə silmək olardı). Əvəzinə cari vəziyyət AYRICA
 * göstərilir və bant riskin yanında oxunur:
 *     "Yüksək · cari mövsümdə risk"
 *
 * Bu, anderraytinq üçün də düzgün siqnaldır: yüksək tarixi bal + cari
 * mövsümdə qırmızı bayraq → ƏLAVƏ YOXLAMA, avtomatik təsdiq deyil.
 * (Qərarı yenə bank verir — bax: faylın başlığı.)
 */
export const CARI_RISK = {
  /** Cari mövsüm amili bu baldan aşağı olmalıdır */
  xalHeddi: 2,
  /** VƏ örtük ətrafdan ən azı bu qədər geri qalmalıdır (NDVI vahidi) */
  ferqHeddi: -0.1,
};

/**
 * Cari mövsümün vəziyyətini baldan AYRI qaytarır. Balı dəyişmir.
 *
 * @returns {{hal, risk, ferq, xal, maxXal, sebeb}}
 *   hal: "yaxsi" | "normal" | "asagi" | "zeif" | "olculmeyib"
 *   risk: hər iki şərt ödənəndə true — bant bu halda tək oxunmamalıdır
 */
export function cariVeziyyetHali(indeks) {
  const setir = indeks?.setirler?.find((s) => s.id === "cariVeziyyet");
  if (!setir?.olculub) {
    // "Ölçülməyib" risk DEYİL: biçilmiş sahə pis becərilmiş sahə deyil
    return { hal: "olculmeyib", risk: false, ferq: null, xal: null, maxXal: null, sebeb: setir?.sebeb ?? null };
  }
  return {
    // Hal bant səbəbindən çıxarılır ki, cədvəl dəyişəndə burada
    // unudulmuş ikinci siyahı qalmasın
    hal: setir.sebeb.split(".")[1],
    risk: setir.xal <= CARI_RISK.xalHeddi && setir.rawValue <= CARI_RISK.ferqHeddi,
    ferq: setir.rawValue,
    xal: setir.xal,
    maxXal: setir.maxXal,
    sebeb: setir.sebeb,
 ...
 *
 * ⚠ BU ÖLÇÜNÜN İKİ MƏLUM ZƏİFLİYİ VAR — hər ikisi balı YUXARI çəkir və
 * hazırda QƏSDƏN düzəldilmir (düzəliş balı dəyişər, bax: TODO-lar):
 *
 * 1. SƏS SAYIDIR, FƏRQİN BÖYÜKLÜYÜ DEYİL. Ətrafı hər mövsüm 0.005 ilə
 *    ötən sahə ilə 0.25 ilə ötən sahə eyni 30/30 alır. "30/30" yalnız
 *    "mövsümlərin ən azı 80%-ində medianın üstündə" deməkdir — "xeyli
 *    yaxşıdır" demək DEYİL. `detal` sahəsi bunu ekranda açıq göstərir.
 *
 * 2. ZİRVƏ AYINI SAHƏ SEÇİR (bax: api/tarixce.js, movsumlereBol). Ətraf
 *    medianı həmin ayda oxunur, amma ətrafın ÖZ zirvəsi başqa ayda ola
 *    bilər: taxıl sahəsi aprel zirvəsində iyulda zirvələyən pambıq
 *    qonşuluğu ilə müqayisə olunur. Yəni sahə öz ən yaxşı anında,
 *    qonşuluq isə təsadüfi anda ölçülür — müqayisə sahənin xeyrinədir.
 *
 * TODO(həmyaş): eyni bitki + oxşar suvarma rejimi olan sahələrlə faiz
 * mövqeyi hər iki zəifliyi birdən aradan qaldırır — bant funksiyası
 * dəyəri onsuz da 0–1 kimi qəbul edir, mühərrik dəyişmir.
 */
function amilNisbiPerformans({ muqayiseli }) {
  ...
  // Median fərqi bal vermir — yalnız izah üçündür: "9/10 mövsüm" rəqəmi
  // "necə 30/30 oldu?" sualını fermerin özü üçün cavablandırır
  const ferqler = muqayiseli.map((m) => m.zirve - m.etrafMedyan);
  return {
    deger: ustde / muqayiseli.length,
    metodologiya,
    detal: { ustde, hamisi: muqayiseli.length, medyanFerq: medyan(ferqler) },
  };
}
...
export function mehsuldarliqIndeksi({ movsumler = [], cari = null, sonIl } = {}) {
  ...
  for (const amil of CEDVEL) {
    const {
      ...
      metodologiya,
      detal = null,
      sebeb: olculmemeSebebi,
    } = HESABLAYICILAR[amil.id](hazir, cari);
