...
/**
 * BAZANIN KİMLİYİ — host + baza adı, PAROLSUZ və istifadəçi adısız.
 *
 * Niyə lazımdır: Vercel preview deployment-lərində Neon hər git branch üçün
 * ayrı baza yaradır. "Miqrasiyanı işlətdim, amma xəta qalır" hallarının
 * əksəriyyəti miqrasiyanın BAŞQA branch-ın bazasına düşməsidir — hansı
 * bazaya bağlandığımızı görmədən bunu tapmaq mümkün deyil.
 *
 * Yalnız SERVER LOGUNA yazılır (bax: api/kredit.js → sxemCavabi), HTTP
 * cavabında getmir: ünvan sirr olmasa da, onu açıq internetə çıxarmırıq.
 * Eyni qayda miqrasiya workflow-unda da var: yalnız host + baza adı.
 */
export function baglantiKimliyi() {
  const unvan = baglantiUnvani();
  if (!unvan) return null;
  try {
    const u = new URL(unvan);
    return u.hostname + u.pathname;
  } catch {
    // Ünvan parse olunmursa MƏZMUNU çap etmirik — parol orada ola bilər
    return "(ünvan oxunmadı)";
  }
}
...
async function musterAl() {
  ...
  try {
    ...
    // SÜRÜCÜNÜN ÖZ XƏTA MƏTNİ BAĞLANTI SƏTRİNİ ÇAP EDİR — parolla birlikdə:
    //   "Database connection string provided to `neon()` is not a valid URL.
    //    Connection string: postgres://user:PAROL@…"
    // Bu mətn build logunda (preview miqrasiyası) və funksiya logunda qalır,
    // log isə paylaşıla bilir. Yuxarıdakı qayda ilə eynidir: yalnız AÇARIN
    // ADI deyilir, dəyər heç vaxt. Orijinal xəta udulur — içində ünvan var.
    const xeta = new Error(
      `Bağlantı sətri oxunmadı (${baglantiAcari()}) — gözlənilən format: postgresql://…`,
    );
    xeta.status = 501;
    throw xeta;
  }
 ...
 * bazada işlədilməyib (Vercel preview-da Neon hər branch üçün AYRI baza
 * yaradır). API-lər bunu 503 `sxemYoxdur` ilə cavablandırır ki, ekran
 * "server sındı" deməsin (bax: api/kredit.js, api/bazar.js).
 */
export function sxemYoxdurXetasi(error) {
  return (
    error?.code === "42P01" ||
