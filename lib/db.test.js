...
describe("db qatı", () => {
  ...
  describe("bağlantı açarı", () => {
    ...
    // Loga hansı bazaya bağlandığımız düşür — parol ORAYA DA düşməməlidir
    it("bağlantı kimliyi host və baza adını verir, parolu vermir", () => {
      process.env.DATABASE_URL = "postgres://istifadeci:parol123@ep-test-123.aws.neon.tech/neondb?sslmode=require";
      const kimlik = baglantiKimliyi();
      expect(kimlik).toBe("ep-test-123.aws.neon.tech/neondb");
      expect(kimlik).not.toMatch(/parol123|istifadeci|sslmode/);
    });

    it("ünvan oxunmursa məzmun çap olunmur", () => {
      process.env.DATABASE_URL = "bu-ünvan-deyil";
      expect(baglantiKimliyi()).toBe("(ünvan oxunmadı)");
    });

    it("bağlantı yoxdursa kimlik də yoxdur", () => {
      expect(baglantiKimliyi()).toBeNull();
    });
  });
});
