// RAYON SİYAHISI — server və klient üçün ORTAQ məlumat.
//
// Əvvəl yalnız `src/services/location.js`-də idi. Bazar sifarişi çatdırılma
// rayonunu SERVERDƏ yoxlamalıdır (anderraytinq məlumatı kimi saxlanılır —
// zibil kod bazaya düşməməlidir), ona görə siyahı asılılıqsız fayla köçdü;
// location.js onu olduğu kimi re-export edir.
//
...
// görünür. Kodlar ASCII-dir ki, URL, analitika və server sorğusunda təhrif
// olunmasın.
...
const KODLAR = new Set(RAYONLAR.map((r) => r.kod));

/** Kod siyahıdadırmı — server girişi yoxlamaq üçün */
export function rayonKoduDuzgun(kod) {
  return typeof kod === "string" && KODLAR.has(kod);
}

export function rayonTap(kod) {
  return RAYONLAR.find((r) => r.kod === kod) ?? null;
}
