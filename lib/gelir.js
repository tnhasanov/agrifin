...
import { kreditUcunHektarDereceleri } from "./subsidiya/index.js";
...
export const GELIR_CONFIG = {
  ...
  /**
   * Dövlət subsidiyası (₼/ha) — KREDİT TAVANINA YALNIZ TƏSDİQLİ MƏBLƏĞ GİRİR.
   *
   * Dərəcələr lib/subsidiya/-dadır (tək mənbə). Cədvəl rəsmi mənbə ilə
   * tutuşdurulmayıbsa və fermerin uyğunluğu təsdiqlənməyibsə buraya 0
   * gəlir: təsdiqsiz subsidiya borc qabiliyyətini şişirtməməlidir.
   * Nəticə: buğda/arpa modeldə subsidiyasız görünür və taxılçının limiti
   * ona görə aşağıdır — bu, qəsdəndir; rəsmi cədvəl və uyğunluq yoxlaması
   * gələndə fərdi hesablamada `kreditUcunSubsidiya` işə düşür.
   */
  subsidiya: kreditUcunHektarDereceleri(),
 ...
 */
function ssenariHesabla({ bitki, hektar, zona, bant, cariRisk, ad, indeksEmsaliKenar = null }) {
  ...
  // Kənardan verilən əmsal (FarmScore v3 multiplikatoru) bant cədvəlini
  // ƏVƏZ EDİR — yalnız FARMSCORE_V3=on rejimində gəlir (bax: lib/kredit.js)
  const indeksEmsali = Number.isFinite(indeksEmsaliKenar) ? indeksEmsaliKenar : (indeksTesiri[bant ?? "yoxdur"] ?? 1);
 ...
 * @param {number|null} [p.indeksEmsali] Kənar multiplikator (FarmScore v3);
 *   verilibsə bant cədvəli əvəzinə bu işlədilir və fərziyyədə mənbəyi yazılır
 *
 * @returns {null | object}
 *   null → bitki və ya sahə bilinmir: TƏXMİN UYDURULMUR
 *   {hal:"hazir", ssenariler, baza, pessimist, optimist, ferziyyeler, yoxlanilib}
 */
export function gelirModeli({ bitki, hektar, zona = "aran", bant = null, cariRisk = false, indeksEmsali = null } = {}) {
  ...
  const kenarEmsal = Number.isFinite(indeksEmsali) ? indeksEmsali : null;
  const ssenariler = SSENARILER.map((ad) =>
    ssenariHesabla({ bitki, hektar, zona, bant, cariRisk, ad, indeksEmsaliKenar: kenarEmsal }),
  );
  ...
  return {
    ...
    ferziyyeler: [
      ...
      kenarEmsal != null
        ? { acar: "indeks", deyer: kenarEmsal, vahid: "×", menbe: "farmScoreV3" }
