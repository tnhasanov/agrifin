...
describe("anderraytinq", () => {
  ...
  it("peyk tarixçəsi olmadan da qərar verilir, amma səbəbdə qeyd olunur", () => {
    ...
    expect(n.girisler.peyk).toEqual({ movsumSayi: 0, cariVar: false, menbe: "yoxdur" });
  ...
  it("hesablama versiyası konfiqurasiyadan çıxarılır", () => {
    expect(HESAB_VERSIYASI).toMatch(/^v1-bal\d+-gelir\d+-odenis\d+-\d+-faiz[\d.]+$/);
  });
});
