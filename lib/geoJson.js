...
/**
 * Sahənin ətrafını da tutan pəncərə — ƏSL RƏNG görüntüsü üçün.
 *
 * Niyə konturla kəsmirik: fermer sahəni yol, arx, ağac sırası və qonşu
 * tarla ilə tanıyır. Bunların hamısı konturdan KƏNARDADIR — sahəni dəqiq
 * kəssək ekranda kontekstsiz ləkə qalır və fermer öz yerini tanımır.
 *
 * `pay` — sahə ölçüsünün neçə qatı əlavə olunsun. Kiçik sahədə faiz azdır,
 * ona görə minimum metr həddi də var.
 */
export function sahePenceresi(noqteler, { pay = 0.6, minMetr = 150 } = {}) {
  ...
  // Minimum əlavə metrlə verilir ki, çox kiçik sahə də kontekst alsın
  const enPay = Math.max((enMax - enMin) * pay, minMetr / 111_320);
  const uzPay = Math.max((uzMax - uzMin) * pay, minMetr / (111_320 * kosinus));
...
/** GeoJSON bbox sırası: [uzunluqMin, enMin, uzunluqMax, enMax] */
export const pencereBbox = (p) => [p.uzMin, p.enMin, p.uzMax, p.enMax];
