 ...
 *
 * Məqsəd: 006 miqrasiyasından ƏVVƏL bazanın yeni invariantlara hazır
 ...
 *
 * Nə yoxlanılır:
 *   1. Bir istifadəçidə birdən çox AKTİV kredit (006 üçün blokerdir);
 ...
 *   4. Klient mənbəli peyk snapshot-larının sayı (006-də 'klient' kimi
...
basliq("1. Aktiv kredit sayı (006 partial unique index üçün)");
if (await cedvelVar("loans")) {
  ...
  if (coxlu.length === 0) {
    ...
    console.log("   006 miqrasiyası bu sətirlər təmizlənmədən tətbiq OLUNMAMALIDIR.");
  }
} else {
...
if (await cedvelVar("saheler")) {
  ...
  if (fərqli === 0) {
    ...
    xeberdarliq += 1;
    console.log(`⚠ ${fərqli} sahədə fərq var (ən böyüyü ${enBoyukFerq.toFixed(1)}%) — 006 backfill onları düzəldəcək.`);
  }
} else {
