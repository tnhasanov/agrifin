 ...
 *   1. FAİZ GÜNDƏLİK HESABLANIR (ACTUAL/365), AYDA BİR DƏFƏ ÖDƏNİŞƏ DÜŞÜR.
 *        faiz = Σ (həmin günün qalıq əsas borcu × illikFaiz/100 / 365)
 *      Dövrün sonunda yığılan cəm bir "interest_charge" hadisəsi kimi
 *      yazılır və həmin gün ödənişə düşür (`due_on`).
 *
 *      "Faiz hər ay ödənilir" ≠ "faiz illik/12-dir": 10.000 @ 12% üçün
 *      31 günlük dövr 101,92 ₼, 30 günlük dövr 98,63 ₼ verir — illik/12
 *      isə hər ay 100 ₼ yazardı. Gündəlik hesablama fermerin ödənişini
 *      TƏBİİ şəkildə mükafatlandırır: əsas borc ayın ortasında azalırsa,
 *      həmin gündən sonrakı günlər artıq yeni qalığa hesablanır — əlavə
 *      "çəkili orta" mexanizmi lazım gəlmir.
 *
 *      HÜQUQİ UYĞUNLUQ: faiz yalnız borcun mövcud olduğu günlərə və qalan
 *      məbləğə hesablanır; 365 bölən məhsul qaydası kimi müqavilədə
 *      sabitlənir. UI-dakı "İlk ayın faizi ~X" isə TƏXMİNDİR (illik/12,
 *      "~" ilə göstərilir) — jurnaldakı faktiki rəqəm buradan çıxır.
 *
...
/** Faiz ili: Actual/365 — faiz yalnız borcun olduğu günlərə hesablanır */
 ...
 *
 * ═══ NİYƏ AYRICA HESAB ═══════════════════════════════════════════════
 * `hesablanacaqDovrler` yalnız TAM bitmiş aylıq dövrləri jurnala yazır
 * (araliq.son > indi → dayanır). Bu, adi ay ortasında düzgündür: faiz
 * ayda bir dəfə ödənişə düşür, ona görə yarımçıq dövr hələ borc deyil.
 *
 * Amma fermer ayın ortasında əsas borcu TAM bağlayırsa, həmin yarımçıq
 * dövrün faizi heç vaxt yazılmır: hər iki balans sıfırlanır və kredit
 * `repaid` olur. Yəni son aylıq tarixdən bağlanma gününə qədərki act/365
 * faizi itir — müqavilə "faiz borcun mövcud olduğu GÜNLƏRƏ hesablanır"
 * deyir, məhsul isə onu almırdı.
 *
 * Bu funksiya həmin aralığı hesablayır. Cərimə YOXDUR — eyni illik dərəcə,
 * eyni act/365 qaydası, sadəcə tam ay yerinə faktiki günlər.
 *
 * @returns {number} qəpiyə yuvarlaqlaşdırılmış faiz (dövr bitibsə 0)
 */
export function payoffFaizi({ xett = [], verilme, hesablanmisDovr = 0, indi, illikFaiz }) {
  if (!verilme) return 0;
  const son = tarixe(indi);
  // Son YAZILMIŞ dövrün sonu — yarımçıq aralığın başlanğıcı budur
  const baslangic = dovrSonu(verilme, hesablanmisDovr);
  if (!(son > baslangic)) return 0;
  return araliqFaizi({ xett, baslangic, son, illikFaiz });
}

/**
 * [başlanğıc, son) aralığında ARTIQ YAZILMIŞ payoff faizi.
 *
 * Erkən tam bağlanma cəhdi yarımçıq dövrün faizini `faiz-payoff-<gün>`
 * açarı ilə jurnala yazır (bax: api/kredit.js → payoffFaiziniYaz). Cəhd
 * krediti bağlamasa (təkrar açar, qırılma, 409), həmin dövr sonradan tam
 * hesablananda eyni günlər İKİNCİ dəfə faizlənərdi. Dövrün faizi bu
 * funksiyanın qaytardığı qədər AZALDILIR: bir gün üçün bir faiz.
 *
 * @returns {number} qəpiyə yuvarlaqlaşdırılmış cəm (yoxdursa 0)
 */
export function yazilmisPayoffFaizi(hadiseler = [], baslangic, son) {
  const b = tarixe(baslangic);
  const s = tarixe(son);
  let cem = 0;
  for (const h of hadiseler) {
    if (h?.event_type !== "interest_charge") continue;
    if (!String(h.idempotency_key ?? "").startsWith("faiz-payoff-")) continue;
    const vaxt = tarixe(h.created_at);
    if (vaxt >= b && vaxt < s) cem += Number(h.amount) || 0;
  }
  return qepik(cem);
}
...
/**
 * [başlanğıc, son) aralığında yığılan faiz — gündəlik, Actual/365.
 *
 *   faiz = Σ(qalıq_i × gün_i) × illikFaiz/100 / 365
 *
 * Qalıq aralıq boyu dəyişməyibsə: 10.000 @ 12% × 31 gün = 101,92 ₼.
 * Qalıq aralığın içində azalıbsa, azalma günündən sonrakı günlər artıq
 * YENİ qalığa hesablanır — əlavə düzəliş mexanizmi lazım deyil.
 *
